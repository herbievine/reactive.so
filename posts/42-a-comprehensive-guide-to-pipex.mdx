...
This program opens a file called `example.txt`, and then uses `dup2(2)` to redirect `stdout` to the file descriptor returned by `open(2)`. This means that any output from `printf(3)` will be written to the file instead of the terminal. The file is then closed, and the `printf(3)` statement writes to the file.
...
The core idea of this program is to mimic the data flow between programs. Running the below scripts in your terminal _should_ output the same data (you need to use double quotes for commands with arguments):
