...
- `mlx_loop_hook`: Hook which gets called each loop (think of it as each frame, like FPS).
- `mlx_key_hook`: Hook which gets called on each keypress.
...
If you want more information about these functions, I recommend visiting 42Docs, as they have done a great job documenting the MLX (linked at the end).

## Installation

Linking the MLX to your code is rather tricky, as it's not very well documented, so I will try to explain it with both a macOS and Linux machine (works on school computers too). These examples assume you have the MLX installed in your project under a folder called `mlx`. If you want to ease the installation process, you can use [42 CLI](https://github.com/herbievine/42-cli), which has great tooling around the MLX.
...
> Note: Always link your external libs AFTER your object files! Not doing so may lead to an `undefined reference`. Learn more about that [here](https://stackoverflow.com/questions/45135/why-does-the-order-in-which-libraries-are-linked-sometimes-cause-errors-in-gcc).
...
This means two things: the player needs access to the exit, and the player, prior to exiting the map, is able to collect all collectables.
...
Before you render something to your window instance, let's first get some basic stuff out of the way.

You first need to go to [itch.io](https://itch.io/game-assets/tag-16x16), and choose some assets for your game (you can also make your own in Photoshop, just make sure you export them as PNG). After that, export each texture into a PNG image, and go to [Convertio](https://convertio.co/png-xpm/), which will take your images and transform them into an XPM file format.
...
typedef struct s_data
{
	...
	t_map		*map; // Map pointer (contains map details - preferably keep on the stack)
}	t_data;
```
...
In this example, you'll notice we set the `mlx_ptr` field in `t_data` to the result of the `mlx_init` function. It may return `NULL` if there was an issue with X11, which you will have to manage.
...
Hooks are an essential part of the MLX, as they allow you to listen for changes and events. Here are a few of the core hooks that you will be using in `so_long`:
...
- `mlx_loop_hook`: This function takes a pointer to a function, which then gets called on every loop.
...
With these three hooks, you will be able to listen for user input and mutate data accordingly.

A basic implementation of this could look like this (note we're not using `mlx_loop_hook` here):
...
- A key release hook, which calls the `on_keypress` function every time the user releases a key. This includes normal keys such as `a`, `A`, and `1`, but also some system keys, such as `ESC`.
...
> The `KeyRelease` and `DestroyNotify` constants can be imported through the X11 headers

### Loading Textures

Loading textures into the MLX is a rather straightforward process. You will be using `mlx_xpm_file_to_image`, which loads a static XPM file from your `/assets` folder. It has the following prototype:
...
- Rendering all textures in every frame. This uses up memory and after a (long) while will cause your program to crash.
- Not using constants to increase readability/maintainability. Although not a bug, it isn't a good habit to explicitly index into an array with a fixed number or to use a plain string as an asset path.
